...
// Company: Rehkopf
// Engineer: Rehkopf
// Adapted for SPC7110 by sd2snes contributor
//
...
// Target Devices:
// Tool versions:
// Description: SPC7110 Top-Level Master Control FSM
//
// Dependencies: address, SPC7110, SPC7110Map
//
// Revision:
...
// SPC7110 enable: active when mapper_id == 7 (3'b111)
reg spc7110_enable;
reg spc7110_reg_enable;
reg spc7110_decomp_enable;
...
begin
  if (SNES_DEADr == 1'b1) begin
    ...
    spc7110_decomp_enable <= 1'b0;
  end else if (MAPPER == 3'b111) begin
    spc7110_enable      <= 1'b1;
    // SPC7110 register range: bank $00/$80, addr $4800-$487F (covers $4840-$484F RTC regs)
    if (SNES_ADDR[22] == 1'b0 && SNES_ADDR[15:8] == 8'h48
        && SNES_ADDR[7] == 1'b0)
      spc7110_reg_enable  <= 1'b1;
    else
      spc7110_reg_enable  <= 1'b0;
    // Decompressed data window: bank $50
    if (SNES_ADDR[23:16] == 8'h50)
      spc7110_decomp_enable <= 1'b1;
    else
      spc7110_decomp_enable <= 1'b0;
  end else begin
    spc7110_enable      <= 1'b0;
...
//--------------------------------------------------------------------
// SPC7110 chip signals
//--------------------------------------------------------------------
wire [7:0] SPC7110_SNES_DATA_OUT;
wire       SPC7110_SNES_DATA_OE;
wire       SPC7110_ROM_REQ;
...
// Single-byte data from PSRAM to SPC7110 chip
assign SPC7110_ROM_DATA = ROM_ADDR0 ? ROM_DATA[7:0] : ROM_DATA[15:8];

// forward declaration — defined with MCU state machine below
wire MCU_HIT;

//--------------------------------------------------------------------
// SPC7110 internal ROM request FSM (free-slot PSRAM access)
//--------------------------------------------------------------------
parameter SPC_IDLE = 2'b00;
parameter SPC_WAIT = 2'b01;
parameter SPC_ACK  = 2'b10;
...
always @(posedge CLK2) begin
  case (spc_state)
    ...
    SPC_WAIT: begin
      spc_delay <= spc_delay - 1;
      if (spc_delay == 4'h0) spc_state <= SPC_ACK;
    end
    SPC_ACK: begin
      spc_state <= SPC_IDLE;
    end
    default: spc_state <= SPC_IDLE;
  endcase
end

assign SPC7110_ROM_ACK = (spc_state == SPC_ACK);
wire spc_rom_active = (spc_state != SPC_IDLE);

// DROM bank/PSRAM address for SNES-direct DROM access ($D0-$FF banks)
wire [2:0] drom_bank_sel =
    (SNES_ADDR[23:20] == 4'hD) ? SPC7110_DROM_BANK_D :
    (SNES_ADDR[23:20] == 4'hE) ? SPC7110_DROM_BANK_E :
                                  SPC7110_DROM_BANK_F;
wire [23:0] snes_drom_psram = {1'b0, (drom_bank_sel + 3'd1), SNES_ADDR[19:0]};
wire is_drom_snes = (SNES_ADDR[23:20] == 4'hD) |
                    (SNES_ADDR[23:20] == 4'hE) |
                    (SNES_ADDR[23:20] == 4'hF);

//--------------------------------------------------------------------
// SPC7110 chip instantiation
//--------------------------------------------------------------------
SPC7110 spc7110_inst (
  .clk            (CLK2),
  .rst            (~spc7110_enable),
  .SNES_ADDR      (SNES_ADDR),
  .SNES_WR        (SNES_WRITE),
  ...
  .DROM_BANK_F    (SPC7110_DROM_BANK_F)
);
...
assign MCU_HIT = MCU_WR_HIT | MCU_RD_HIT;
...
assign ROM_ADDR  = (SD_DMA_TO_ROM) ? MCU_ADDR[23:1]
            ...
            : spc_rom_active ? SPC7110_ROM_ADDR[23:1]
            : (spc7110_enable & is_drom_snes) ? snes_drom_psram[23:1]
...
assign ROM_ADDR22 = (SD_DMA_TO_ROM) ? MCU_ADDR[1]
            : MCU_HIT ? ROM_ADDRr[1]
            : spc_rom_active ? SPC7110_ROM_ADDR[1]
            : (spc7110_enable & is_drom_snes) ? snes_drom_psram[1]
            : MAPPED_SNES_ADDR[1];

assign ROM_ADDR  = (SD_DMA_TO_ROM) ? MCU_ADDR[23:2]
            : MCU_HIT ? ROM_ADDRr[23:2]
            : spc_rom_active ? SPC7110_ROM_ADDR[23:2]
            : (spc7110_enable & is_drom_snes) ? snes_drom_psram[23:2]
...
assign ROM_ADDR0 = (SD_DMA_TO_ROM) ? MCU_ADDR[0]
            : MCU_HIT ? ROM_ADDRr[0]
            : spc_rom_active ? SPC7110_ROM_ADDR[0]
            : (spc7110_enable & is_drom_snes) ? snes_drom_psram[0]
            : MAPPED_SNES_ADDR[0];
...
always @(posedge CLK2) begin
  ...
  // RG make sure the MCU doesn't touch the PSRAM when SDD1 may be accessing it
      if((free_slot & ~spc_rom_active) | SNES_DEADr) begin
...
assign ROM_DATA[7:0] = ROM_ADDR0 ?
                ...
                (SD_DMA_TO_ROM ? (!MCU_WRITE_1 ? MCU_DOUT : 8'bZ)
                  : MCU_WR_HIT ? MCU_DOUT
                  // SPC7110 never writes to PSRAM ROM; SRAM writes use normal path
                  : (ROM_HIT & IS_WRITABLE & ~SNES_WRITE) ? SNES_DATA
                  : 8'bZ )
                // if ROM_ADDR[0] = '0'
                :8'bZ;
...
// byte selectors for PSRAM; SPC7110 uses single-byte (8-bit) accesses
...
assign SNES_DATABUS_OE = msu_enable & ~(SNES_READ_narrow & SNES_WRITE) ? 1'b0 :
                         ...
                         (spc7110_reg_enable | spc7110_decomp_enable) ? 1'b0 :
